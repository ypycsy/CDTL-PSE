...
def test_ner(results, path):
    ...
    with codecs.open(output_file,"w",encoding="utf-8") as f:
        ...
        f.writelines(to_write)
    eval_lines,results_ner = return_report(output_file)
    return eval_lines
...
def clean(params):
    ...
    if os.path.isfile(params.vocab_file):
        #print('params.vocab_file',params.vocab_file)
        os.remove(params.vocab_file)

    if os.path.isfile(params.map_file):
        #print('params.vocab_file', params.map_file)
...
def save_model(sess, model, path, logger):
    ...
    saves=model.saver.save(sess, checkpoint_path)
    model.saver.restore(sess, checkpoint_path)
    #saves=sess.run(model.)
    #print('saves',saves)
    logger.info("model saved")


def create_model( Model_class, path, load_vec, config, id_to_char,id_to_char_loc,id_to_char_per,id_to_char_org, logger):
    ...
    #with tf.Session(config=tf_config, graph = model.graph) as session:
        #session.run(tf.global_variables_initializer())
        #if config["pre_emb"]:
                #emb_weights = session.run(model.char_lookup.read_value())
                #emb_weights_ner = session.run(model.char_lookup.read_value())
                #print('emb_weights',emb_weights)
                #emb_weights,emb_weights_ner = load_vec(config["emb_file"],id_to_char,id_to_char_ner, config["char_dim"], emb_weights, emb_weights_ner)
                #emb_weights_ner = load_vec(config["emb_file"],id_to_char, id_to_char_ner, config["char_dim"], emb_weights_ner)
                #print('emb_weights', emb_weights)
                #session.run(model.char_lookup.assign(emb_weights))
                #session.run(model.char_lookup_ner.assign(emb_weights_ner))
                #aa=session.run(model.char_lookup.assign(emb_weights))
                #print('aaaaaa',aa)
...
def result_to_json(string, tags):
    ...
    for char, tag in zip(string, tags):
        if tag[0] == "S":
            ...
            item.append( entity_name)
            #print('item',item["entities"])
            entity_name = ""
        else:
            entity_name = ""
            entity_start = idx
