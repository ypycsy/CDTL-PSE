...
def load_sentences(path, lower, zeros):
    ...
    if len(sentence) > 0:
        if 'DOCSTART' not in sentence[0][0]:
            sentences.append(sentence)
    return sentences
def load_sentences_test(path, lower, zeros):
    """
    Load sentences. A line must contain at least a word and its tag.
    Sentences are separated by empty lines.
    """
...
def prepare_dataset(sentences, char_to_id, tag_to_id, lower=False, train=True):
    ...
    return data
def prepare_dataset_ner(sentences, char_to_id_ner, tag_to_id_ner, lower=False, train=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
    ...
    for s in sentences:
        num+=1
        string = [w[0] for w in s]
        chars = [char_to_id_ner[f(w) if f(w) in char_to_id_ner else '<UNK>']
                 for w in string]
        segs = get_seg_features("".join(string))
        if train:
